<script lang="ts">
	import { Download, KeyRound, Link2, MoreHorizontal, Pencil, Power, PowerOff, QrCode, Trash2 } from 'lucide-svelte';
	...
	const items = $derived<MenuItem[]>([
		{ label: 'Show QR code', icon: QrCode, onselect: () => onaction('qr', peer), disabled: !peer.has_private_key },
		{ label: 'Download config', icon: Download, onselect: () => onaction('download', peer), disabled: !peer.has_private_key },
		{ label: 'Share link', icon: Link2, onselect: () => onaction('share', peer), hidden: !canWrite },
		{ label: 'Edit', icon: Pencil, onselect: () => onaction('edit', peer), separator: true, hidden: !canWrite },
		{
		...
		{ label: 'Delete', icon: Trash2, danger: true, separator: true, onselect: () => onaction('delete', peer), hidden: !canWrite }
	]);
</script>
