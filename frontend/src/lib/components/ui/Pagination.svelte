...
<nav class="flex flex-wrap items-center justify-between gap-3 text-sm text-fg-muted" aria-label="Pagination">
	<p class="tabular">
		{#if total === 0}
			No results
		{:else}
			Showing <span class="font-medium text-fg">{from}–{to}</span> of
			<span class="font-medium text-fg">{total.toLocaleString()}</span>
		{/if}
	</p>
	<div class="flex items-center gap-2">
		...
		<span class="text-fg tabular">{page} / {pages}</span>
		<IconButton
			label="Next page"
			variant="outline"
			size="sm"
