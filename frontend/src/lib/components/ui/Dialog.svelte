...
>
	{#if open}
		<div class="fixed inset-0 flex items-end justify-center p-0 sm:items-center sm:p-6">
			...
			>
				<header class={`flex items-start justify-between gap-4 px-5 pt-5 ${header ? 'pb-0' : 'pb-3'} ${hideTitle ? 'sr-only' : ''}`}>
					<div class="min-w-0">
					...
					{#if !locked}
						<IconButton label="Close" size="sm" onclick={requestClose} class="-mr-2 -mt-1">
							<X class="h-4 w-4" />
						</IconButton>
					{/if}
				</header>
				...
				{/if}
				<div bind:this={body} class="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-5 py-4" tabindex="-1">
					{@render children()}
				</div>
				{#if footer}
					<footer class="flex flex-col-reverse gap-2 border-t border-border bg-bg-subtle/60 px-5 py-3 sm:flex-row sm:justify-end">
						{@render footer()}
					</footer>
				{/if}
			</div>
