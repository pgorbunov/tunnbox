...
<script lang="ts" generics="T, K extends string | number">
	...
	// Cards below 768px, and also whenever the table's own container is too
	// narrow for the columns (e.g. a tablet with the sidebar open).
	...
	const useCards = $derived(!!card && (!desktop.current || containerWidth < 600));
							...
							>
								{#if col.sortable}
									<button
										type="button"
										onclick={() => sortBy(col)}
										class={`inline-flex items-center gap-1 rounded-sm uppercase hover:text-fg ${sortKey === col.key ? 'text-fg' : ''}`}
									>
										{col.label}
										{#if sortKey === col.key}
