...
from app.schemas.mfa import MfaDisableRequest, MfaEnableRequest, MfaPasswordRequest, MfaSetupResponse, RecoveryCodes
...
async def setup(principal: SessionPrincipal, ctx: Ctx) -> MfaSetupResponse:
    return MfaSetupResponse(**await mfa_service.setup(ctx, principal))
...
async def enable(body: MfaEnableRequest, principal: SessionPrincipal, ctx: Ctx) -> RecoveryCodes:
    return RecoveryCodes(recovery_codes=await mfa_service.enable(ctx, principal, body.code))
